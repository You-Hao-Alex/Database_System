...
public class hashload {
	...
	private void generate(int pagesize) {
		...
		try {
			ArrayList<Hashtables> tables = new ArrayList<Hashtables>(hashtablesize);
			...
			// Read data from heap file
			while (hashdata != null) {
				for (int j = 0; j < (hashdata.size()); j++) {
					// Save in current bucket
					if ((tables.get(HashFunctions.gethashcode(((hashdata.get(j)).getBNname()), hashtablesize))
							.size() < bucketsize)) {
					...
					// Overflow to next bucket
						...
						int number = HashFunctions.gethashcode("" + ((hashdata.get(j)).getBNname()), hashtablesize);
						do {
							if (number == lasthash)
								number = 0;
							else
