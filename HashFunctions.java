		...
		try {
			...
			if (pageno == null) {
				...
				if (hashcode == lasttable) {
					...
					times++;
					int temphashcode = hashcode - 1; // Save the current file number
					hashcode++; // Hash code for next search
					if (times == overflow)
						pagenumber = fail;
					else {
						System.out.println("There is no result in hash" + temphashcode + ".4096");
